...
export function viewingActionsFor(item) {
  ...
  if (item.state === "Watched")
    return [
      { key: "watch-again", label: "↻ Watch again", state: "In Progress" },
    ];
...
export default function WatchableActionMenu({
  item,
  targetId,
  onTarget,
  onViewingAction,
  ...
  return (
    <div
      className={`nodeContextMenu watchableActionMenu ${variant === "inline" ? "inlineActionMenu" : ""} ${className}`}
      style={style}
    ...
    >
      ...
      {!item.personal && onTarget && (
        <button
          ...
          title={item.id === targetId ? "Already the active target" : ""}
          onClick={() => run(() => onTarget(item))}
        >
          ◎ Mark as target
        </button>
      )}
      ...
      {onInspect && (
        <>
          <hr />
          <button role="menuitem" onClick={() => run(() => onInspect(item))}>
            ⓘ Inspect details
          </button>
        </>
      )}
