...
function textMatches(value, model) {
  const actual = String(value ?? "").toLowerCase(),
    needle = String(model.filter ?? "").toLowerCase();
  switch (model.type) {
    case "equals":
...
function numberMatches(value, model) {
  const actual = Number(value),
    ...
    filterTo = Number(model.filterTo);
  ...
  switch (model.type) {
    ...
    case "greaterThanOrEqual":
      return actual >= filter;
    case "inRange":
      return actual >= filter && actual <= filterTo;
    case "blank":
...
function scalarMatches(value, model) {
  ...
  if (model.filterType === "date")
    return numberMatches(Date.parse(value), {
      ...model,
      filter: Date.parse(model.dateFrom),
      filterTo: Date.parse(model.dateTo),
    });
  if (model.filterType === "set")
    return !model.values?.length || model.values.includes(value);
...
function applyFilterModel(rows, filterModel) {
  ...
  return entries.length
    ? rows.filter((row) =>
        entries.every(([field, model]) => scalarMatches(row[field], model)),
      )
    : rows;
}
...
export function createInfiniteDatasource(
  rows,
  { quickFilter = "", allowSort = true } = {},
) {
  const snapshot = [...rows],
    query = quickFilter.trim().toLowerCase();
  return {
    getRows(params) {
      let result = query
        ? snapshot.filter((row) =>
      ...
      const block = result.slice(params.startRow, params.endRow),
        lastRow = params.endRow >= result.length ? result.length : -1;
